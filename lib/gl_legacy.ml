...
let lib_gl =
  let trylib filename =
    Some (Dl.dlopen ~flags:[ Dl.RTLD_NOW; Dl.RTLD_GLOBAL ] ~filename)
  in
  match Sys.os_type with
  | "Win32" | "Cygwin" -> trylib "opengl32.dll"
  | "Unix" -> begin
  ...
  | _ ->
      print_endline "Unsupported OS type.";
      None

(* ---------------------------------------------------------------------- *)
(* Helpers to reduce boilerplate *)

let foreign0 name = foreign ?from:lib_gl name (void @-> returning void)
let foreign1 name t1 = foreign ?from:lib_gl name (t1 @-> returning void)
...
let foreign5 name t1 t2 t3 t4 t5 =
  foreign ?from:lib_gl name (t1 @-> t2 @-> t3 @-> t4 @-> t5 @-> returning void)
...
let ortho =
  foreign6 "glOrtho" C.double C.double C.double C.double C.double C.double

let frustum =
  foreign6 "glFrustum" C.double C.double C.double C.double C.double C.double

let viewport = foreign4 "glViewport" C.int C.int C.int C.int
...
let clear_color = foreign4 "glClearColor" C.float C.float C.float C.float
...
let render_mode = foreign "glRenderMode" (enum @-> returning C.int)
let render_mode mode = render_mode (render_mode_enum mode)
...
*)
  ...
  *)
  ...
  let feedback_buffer = foreign3 "glFeedbackBuffer" C.int enum ba_as_float32p

  (* void glPassThrough(GLfloat token)
     https://registry.khronos.org/OpenGL-Refpages/gl2.1/xhtml/glPassThrough.xml*)
  let pass_through = foreign1 "glPassThrough" C.float
...
let new_list list mode = new_list list (list_mode_enum mode)
